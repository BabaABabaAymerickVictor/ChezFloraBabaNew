...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['user_id']) && isset($_POST['is_deleted'])) {
    $userId = $_POST['user_id'];
    $isDeleted = $_POST['is_deleted'] === '1' ? 1 : 0;

    try {
        $query = "UPDATE user SET is_deleted = :is_deleted WHERE id_user = :id_user";
...
try {
    $query = "SELECT id_user, email, is_deleted FROM user ORDER BY id_user";
    $stmt = $conn->prepare($query);
    $stmt->execute();
...
<head>
    ...
    <title>ChezFlora - Gestion des Utilisateurs</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    <!-- Contenu principal -->
    <div class="users-container animate__animated animate__fadeInUp">
        <div class="floral-decoration floral-top-right"></div>
        <div class="floral-decoration floral-bottom-left"></div>
        
        <h2 class="animate__animated animate__fadeIn">Gestion des Utilisateurs</h2>
                ...
                <tbody>
                    <?php foreach ($users as $user): ?>
                        <tr>
                            ...
                            <td>
                                <span class="<?php echo $user['is_deleted'] == 0 ? 'status-active' : 'status-inactive'; ?>">
                                    <?php echo $user['is_deleted'] == 0 ? 'Actif' : 'Désactivé'; ?>
                                </span>
                            </td>
                            <td>
                                <form method="POST" action="users.php" style="display: inline;">
                                    <input type="hidden" name="user_id" value="<?php echo $user['id_user']; ?>">
                                    <input type="hidden" name="is_deleted" value="<?php echo $user['is_deleted'] == 0 ? '1' : '0'; ?>">
                                    <label class="switch">
                                        <input type="checkbox" <?php echo $user['is_deleted'] == 0 ? 'checked' : ''; ?> onchange="this.form.submit()">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            usersContainer.addEventListener('mouseenter', function() {
