...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['service_id']) && isset($_POST['is_deleted'])) {
    $serviceId = $_POST['service_id'];
    $isDeleted = $_POST['is_deleted'] === '1' ? 1 : 0;

    try {
        $query = "UPDATE services SET is_deleted = :is_deleted WHERE id_service = :id_service";
        $stmt = $conn->prepare($query);
        $stmt->bindParam(':is_deleted', $isDeleted, PDO::PARAM_INT);
        $stmt->bindParam(':id_service', $serviceId, PDO::PARAM_INT);
        $stmt->execute();

        $successMessage = 'Statut du service mis à jour avec succès.';
...
try {
    $query = "SELECT id_service, nom_service, description, image_service, is_deleted 
              FROM services 
              ORDER BY id_service";
    $stmt = $conn->prepare($query);
    $stmt->execute();
...
<head>
    ...
    <title>ChezFlora - Gestion des Services</title>
    <!-- Bootstrap CSS -->
    ...
    <style>
        ...
        .services-table th:nth-child(1), .services-table td:nth-child(1) { /* ID */
            width: 5%;
        }

        .services-table th:nth-child(2), .services-table td:nth-child(2) { /* Image */
            width: 10%;
        }

        .services-table th:nth-child(3), .services-table td:nth-child(3) { /* Nom */
            width: 20%;
        }

        .services-table th:nth-child(4), .services-table td:nth-child(4) { /* Description */
            width: 35%; /* Réduit pour équilibrer avec les autres colonnes */
        }

        .services-table th:nth-child(5), .services-table td:nth-child(5) { /* Statut */
            width: 10%;
        }

        .services-table th:nth-child(6), .services-table td:nth-child(6) { /* Actions */
            width: 20%;
        }
        ...
        .edit-btn {
            ...
            border: none;
            border-radius: 20px;
            padding: 5px 10px; /* Réduit pour un bouton plus compact */
            color: white;
            font-size: 0.9rem;
            transition: all 0.3s ease;
...
<body>
    ...
    <!-- Contenu principal -->
    <div class="services-container animate__animated animate__fadeInUp">
        <div class="floral-decoration floral-top-right"></div>
        <div class="floral-decoration floral-bottom-left"></div>
        
        <h2 class="animate__animated animate__fadeIn">Gestion des Services</h2>
        
        <!-- Bouton pour ajouter un service -->
        <button type="button" class="add-service-btn" data-bs-toggle="modal" data-bs-target="#addServiceModal">
                ...
                <tbody>
                    <?php foreach ($services as $service): ?>
                        <tr>
                            ...
                            <td><img src="<?php echo htmlspecialchars($service['image_service']); ?>" alt="<?php echo htmlspecialchars($service['nom_service']); ?>" class="service-image"></td>
                            <td><?php echo htmlspecialchars($service['nom_service']); ?></td>
                            <td><?php echo htmlspecialchars($service['description']); ?></td>
                            <td>
                                <span class="<?php echo $service['is_deleted'] == 0 ? 'status-active' : 'status-inactive'; ?>">
                                    <?php echo $service['is_deleted'] == 0 ? 'Actif' : 'Désactivé'; ?>
                                </span>
                            </td>
                            <td>
                                <form method="POST" action="services.php" style="display: inline;">
                                    ...
                                    <input type="hidden" name="is_deleted" value="<?php echo $service['is_deleted'] == 0 ? '1' : '0'; ?>">
                                    <label class="switch">
                                        <input type="checkbox" <?php echo $service['is_deleted'] == 0 ? 'checked' : ''; ?> onchange="this.form.submit()">
                                        <span class="slider"></span>
                                    </label>
                                </form>
                                <a href="update_service.php?id=<?php echo $service['id_service']; ?>" class="edit-btn ms-2">
    ...
    <!-- Modale pour ajouter un service -->
